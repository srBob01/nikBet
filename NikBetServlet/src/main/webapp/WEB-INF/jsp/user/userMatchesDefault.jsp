...
<head>
    ...
    <style>
        ...
        .game-item {
            ...
            margin: 5px;
            padding: 20px;
            width: calc(50% - 10px); /* Рассчитываем ширину так, чтобы учитывать отступы */
...
<a href="<c:url value='/user/matches/default'/>" class="fixed-button-right">
...
<c:if test="${not empty requestScope.predictionResultDto}">
    <div class="message-success">
        <p>You bet on the match: ${requestScope.predictionResultDto.homeTeam()} vs
                ${requestScope.predictionResultDto.guestTeam()}
        </p>
        <p>Expected result: ${requestScope.predictionResultDto.winner()}</p>
        <p>Expected gain: ${requestScope.predictionResultDto.possibleWin()}</p>
    </div>
</c:if>
